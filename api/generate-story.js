...
async function searchCharacterInfo(characterName, apiKey) {
    const response = await fetch('https://google.serper.dev/search', {
        ...
        body: JSON.stringify({ q: `${characterName} 캐릭터 강점 약점 배경 능력` })
    });
    if (!response.ok) {
...
export default async function handler(req, res) {
    ...
    try {
        // Promise.all을 사용해 두 캐릭터 정보를 동시에 검색하여 시간 단축
        const [info1, info2] = await Promise.all([
            searchCharacterInfo(char1, serperApiKey),
            searchCharacterInfo(char2, serperApiKey)
        ]);
